...
class VideoProcessor {
 public:
  ...
  VideoProcessor(webrtz::VideoEncoder* encoder,
 ...
 private:
   ...
   private:
    class EncodeCallbackTask : public rtc::QueuedTask {
     ...
     private:
      ...
      rtc::Buffer buffer_;
      webrtz::EncodedImage encoded_image_;
      const webrtz::CodecSpecificInfo codec_specific_info_;
    };
  ...
  class VideoProcessorDecodeCompleteCallback
   ...
   public:
    ...
    int32_t Decoded(webrtz::VideoFrame& image) override {
      // Post the callback to the right task queue, if needed.
      if (!task_queue_->IsCurrent()) {
    ...
    int32_t Decoded(webrtz::VideoFrame& image,
                    int64_t decode_time_ms) override {
      return Decoded(image);
    }

    void Decoded(webrtz::VideoFrame& image,
  ...
  void FrameEncoded(const webrtz::EncodedImage& encoded_image,
                    const webrtz::CodecSpecificInfo& codec_specific);

  // Invoked by the callback adapter when a frame has completed decoding.
  void FrameDecoded(const webrtz::VideoFrame& image, size_t simulcast_svc_idx);
  ...
  const webrtz::EncodedImage* BuildAndStoreSuperframe(
  ...
  // Codecs.
  webrtz::VideoEncoder* const encoder_;
