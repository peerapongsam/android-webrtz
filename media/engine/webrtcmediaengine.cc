...
#ifdef HAVE_WEBRTC_VIDEO
  typedef WebRtcVideoEngine VideoEngine;
  std::tuple<std::unique_ptr<webrtz::VideoEncoderFactory>,
...
void DiscardRedundantExtensions(
  ...
  for (const char* uri : extensions_decreasing_prio) {
    auto it = std::find_if(
        ...
        [uri](const webrtz::RtpExtension& rhs) { return rhs.uri == uri; });
...
std::vector<webrtz::RtpExtension> FilterRtpExtensions(
  ...
  std::vector<webrtz::RtpExtension> result;
  ...
  std::sort(result.begin(), result.end(),
            [](const webrtz::RtpExtension& rhs,
               const webrtz::RtpExtension& lhs) {
  ...
  if (filter_redundant_extensions) {
    auto it = std::unique(
        ...
        [](const webrtz::RtpExtension& rhs, const webrtz::RtpExtension& lhs) {
    ...
    static const char* const kBweExtensionPriorities[] = {
        ...
        webrtz::RtpExtension::kTimestampOffsetUri};
...
webrtz::BitrateConstraints GetBitrateConfigForCodec(const Codec& codec) {
  webrtz::BitrateConstraints config;
  int bitrate_kbps = 0;
